...
const user = {
  name: "Chinglu", tier: "M", progressPct: 62,
...
const history = [
  ...
  { date: "2023.04.01", from: "—", to: "N", volume: "—",      note: "帳號建立，初始等級" },
];
...
function CurrentStatusCard() {
  const tier = tiers.find((t) => t.id === "m")!;
  const nextTier = tiers.find((t) => t.id === "lu")!;
  return (
    <div style={{ background: "linear-gradient(135deg, #1a1508 0%, #0e0e12 40%, #0a0a0b 100%)", border: `0.5px solid ${C.borderMid}`, borderRadius: 16, padding: 32, position: "relative", overflow: "hidden", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 32 }} className="grid-2">
      ...
      <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
          {[{ label: "商城折扣", value: tier.discount }].map((s) => (
            <div key={s.label}>
      ...
      <div style={{ display: "flex", flexDirection: "column", justifyContent: "center", gap: 16 }}>
        ...
        <div style={{ fontSize: 11, color: C.textMuted, fontFamily: F.body, lineHeight: 1.6 }}>
          達到 <span style={{ color: nextTier.color }}>Lu</span> 等級可獲得：專屬客戶經理、私人交易訊號、50% 商城折扣
        </div>
      ...
      {tiers.map((tier) => {
        ...
        return (
          ...
          >
            ...
            {tier.id === "lu" && (
              <div style={{ marginTop: 14, padding: "8px 12px", background: "rgba(232,201,106,0.06)", border: `0.5px solid ${tier.colorBorder}`, borderRadius: 6 }}>
...
function BenefitsTable() {
  const rows = [
    { label: "月交易量門檻",       values: ["$0+", "$500K+", "$1.5M+", "$4M+ / Founder"] },
      ...
      <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: F.body }}>
        <thead>
          <tr style={{ borderBottom: `0.5px solid ${C.borderSubtle}` }}>
            ...
            {tiers.map((t, i) => (
              <th key={t.id} style={{ padding: "16px 20px", textAlign: "center", fontSize: 18, color: tierColors[i], fontWeight: 300, fontFamily: F.display, letterSpacing: 2, background: t.id === "m" ? "rgba(201,168,76,0.04)" : "transparent" }}>
                {t.name}
              </th>
            ))}
          </tr>
  ...
  return (
    <Layout activePath="/vip" title="VIP Membership">
      <div style={{ display: "flex", flexDirection: "column", gap: 32, maxWidth: 1200 }}>
        ...
        <div>
          <SectionLabel>等級總覽 · N · O · M · Lu</SectionLabel>
          <TierCards />
        </div>
        <div>
