...
const tiers = [
  ...
  },
  {
    ...
    label: "銀牌",
    icon: "◈",
    requirement: "$500K – $1.5M",
    color: "#a8a9ad",
    colorDim: "rgba(168,169,173,0.15)",
  ...
  },
  {
    ...
    requirement: "$4M+",
    color: "#E8C96A",
    colorDim: "rgba(232,201,106,0.15)",
    colorBorder: "rgba(232,201,106,0.4)",
    commission: "0.08%",
    pointsRate: "3.0×",
    discount: "50%",
    support: "專屬經理",
  ...
  return (
    <span
      style={{
        ...
        padding: "3px 10px",
        borderRadius: 20,
        border: `0.5px solid ${tier.colorBorder}`,
    ...
    >
      ...
      <div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
          <div style={{ fontFamily: F.display, fontSize: 42, fontWeight: 300, color: C.goldLight, letterSpacing: 2, lineHeight: 1 }}>
            {tier.name}
          </div>
          <TierBadge tier={tier} />
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
          {[
            { label: "佣金費率",  value: tier.commission },
            { label: "積分倍率",  value: tier.pointsRate },
      ...
      <div style={{ display: "flex", flexDirection: "column", justifyContent: "center", gap: 16 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: C.textMuted, fontFamily: F.mono }}>
          <span>目前 {user.current}</span>
          <span>{user.progressPct}%</span>
          <span>目標 {user.target}</span>
        </div>
        <div style={{ fontSize: 11, color: C.textMuted, fontFamily: F.body, lineHeight: 1.6 }}>
          達到 <span style={{ color: nextTier.color }}>Founder</span> 等級可獲得：
          專屬客戶經理、私人交易訊號、3× 積分倍率、50% 商城折扣
        </div>
      </div>
    </div>
  );
      ...
      {tiers.map((tier) => {
        ...
        return (
          <div
            ...
            style={{
              background: isCurrent ? "rgba(201,168,76,0.06)" : isHov ? C.bgCardHover : C.bgCard,
              border: `0.5px solid ${isCurrent ? C.borderStrong : isHov ? C.borderMid : C.borderSubtle}`,
              borderRadius: 14,
              padding: 24,
              position: "relative",
              overflow: "hidden",
          ...
          >
            {/* 目前等級標籤 */}
            {isCurrent && (
              <div style={{ position: "absolute", top: 12, right: 12, fontSize: 9, color: C.gold, background: "rgba(201,168,76,0.1)", border: `0.5px solid ${C.borderMid}`, padding: "2px 8px", borderRadius: 4, letterSpacing: 1, fontFamily: F.body }}>
                目前
              </div>
            )}
...
<ellipse cx="14" cy="19" rx="10" ry="12" fill="#C9A84C" opacity="0.4"/>
    ...
    <span style={{ fontSize: 28, color: tier.color }}>{tier.icon}</span>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              {[
                { label: "私人訊號", value: tier.signals },
                { label: "專屬經理", value: tier.manager },
                { label: "Founder Pass", value: tier.founderPass },
              ].map((s) => (
                <div key={s.label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span style={{ fontSize: 11, color: C.textMuted, fontFamily: F.body }}>{s.label}</span>
                  <span style={{ fontSize: 13, color: s.value ? "#5ea96e" : C.textMuted }}>
                    {s.value ? "✓" : "✗"}
                  </span>
                </div>
              ))}
      ...
      <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: F.body }}>
        <thead>
          <tr style={{ borderBottom: `0.5px solid ${C.borderSubtle}` }}>
            <th style={{ padding: "16px 20px", textAlign: "left", fontSize: 11, color: C.textMuted, fontWeight: 400, letterSpacing: 1.5, textTransform: "uppercase", width: "30%" }}>
              權益項目
            </th>
            {tiers.map((t, i) => (
                ...
                return (
                  <td
                    ...
                    style={{
                      padding: "14px 20px",
                      textAlign: "center",
                      fontFamily: isCross || isCheck ? F.body : F.mono,
                      fontSize: isCheck || isCross ? 14 : 12,
                      fontWeight: 700,
                      color: isCross ? C.textMuted : isCheck ? "#5ea96e" : isCurrent ? C.gold : tierColors[vi],
            ...
            <div key={i} style={{ display: "flex", gap: 20, position: "relative" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", flexShrink: 0 }}>
                <div
                  style={{
                    width: 32, height: 32,
                    borderRadius: "50%",
                    background: toTier ? toTier.colorDim : "rgba(255,255,255,0.04)",
                    border: `0.5px solid ${toTier ? toTier.colorBorder : C.borderSubtle}`,
                    display: "flex", alignItems: "center", justifyContent: "center",
                    fontSize: 13, color: toTier ? toTier.color : C.textMuted,
                    flexShrink: 0,
                  }}
                >
                  {toTier ? toTier.icon : "◎"}
                ...
                {i < history.length - 1 && (
                  <div style={{ width: 1, flex: 1, minHeight: 32, background: C.borderSubtle, margin: "4px 0" }} />
                )}
              </div>
              ...
              <div style={{ flex: 1, paddingBottom: i < history.length - 1 ? 24 : 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
                  ...
                  {h.from !== "—" && (
                    <>
                      <span style={{ fontSize: 11, color: C.textMuted }}>{h.from}</span>
                ...
                {h.volume !== "—" && (
                  <div style={{ fontSize: 11, color: C.textMuted, fontFamily: F.mono }}>達成交易量：{h.volume}</div>
                )}
              </div>
            </div>
          );
      ...
      <div style={{ display: "flex", minHeight: "100vh", background: "#0a0a0b" }}>
        ...
        <aside style={{ width: 240, minHeight: "100vh", background: "rgba(14,14,18,0.97)", borderRight: `0.5px solid rgba(201,168,76,0.12)`, position: "fixed", top: 0, left: 0, zIndex: 100, display: "flex", flexDirection: "column", paddingBottom: 24 }}>
          ...
          <nav style={{ flex: 1, padding: "20px 12px", display: "flex", flexDirection: "column", gap: 2 }}>
            {[
              ...
              { label: "VIP Membership", icon: "◈", href: "/vip",         active: true  },
          ...
          <div style={{ padding: "16px 12px 0", borderTop: `0.5px solid rgba(201,168,76,0.12)` }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", borderRadius: 8, border: `0.5px solid rgba(201,168,76,0.12)` }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 10, color: "#4a4740", fontFamily: F.mono }}>Bybit</div>
              </div>
            </div>
          </div>
        </aside>
        ...
        <main style={{ marginLeft: 240, flex: 1, minHeight: "100vh" }}>
          ...
          <header style={{ height: 64, borderBottom: `0.5px solid rgba(201,168,76,0.12)`, display: "flex", alignItems: "center", padding: "0 32px", position: "sticky", top: 0, zIndex: 50, background: "rgba(10,10,11,0.92)", backdropFilter: "blur(20px)" }}>
            <div>
              <div style={{ fontFamily: F.display, fontSize: 22, fontWeight: 400, color: "#f0ece0", letterSpacing: 0.5 }}>VIP 會員等級</div>
            </div>
            <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 10px", borderRadius: 8, border: `0.5px solid rgba(201,168,76,0.12)` }}>
                ...
                <span style={{ fontFamily: F.mono, fontSize: 11, color: "#8a8578" }}>Bybit</span>
              </div>
            </div>
          </header>
